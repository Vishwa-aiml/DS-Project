...
import { AnalyticsSubTab, ZoneData } from '../types';
import { 
  BarChart, 
  Bar, 
  XAxis, 
...
export const AnalyticsView: React.FC<AnalyticsViewProps> = ({ zones }) => {
  ...
  // Compute LST vs NDVI scatter points dynamically
  const scatterPoints = useMemo(() => {
    return zones.map(z => ({
      ndvi: Number(z.ndviScore?.toFixed(3) || 0),
      lst: Number(z.lstTemp?.toFixed(1) || 0),
      name: z.name
    })).filter(p => p.ndvi > 0 && p.lst > 0).slice(0, 50); // limit to avoid clutter
  }, [zones]);

  // Compute Temperature Distribution dynamically
  const tempDistributionData = useMemo(() => {
    if (!zones.length) return [];
    
    // Group temperatures into brackets
    const brackets = {
      '< 34°C': 0,
      '34-36°C': 0,
      '36-38°C': 0,
    ...
    zones.forEach(z => {
      const t = z.airTemp;
      if (t < 34) brackets['< 34°C']++;
      else if (t < 36) brackets['34-36°C']++;
      else if (t < 38) brackets['36-38°C']++;
      else if (t < 40) brackets['38-40°C']++;
      else if (t < 42) brackets['40-42°C']++;
      else brackets['42°C+']++;
    });
    
    return Object.entries(brackets).map(([temp, days]) => ({ temp, days }));
  }, [zones]);
    ...
    <div id="analytics-view" className="max-w-7xl mx-auto space-y-6 pb-16 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        >
          ...
          <div className="h-64 sm:h-72 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={tempDistributionData} margin={{ top: 10, right: 20, bottom: 20, left: 0 }}>
