...
export const RealLeafletMap: React.FC<RealLeafletMapProps> = ({
  ...
  useEffect(() => {
    ...
    zones.forEach((zone) => {
      ...
      if (zone.geoJsonPolygon) {
        // Render as GeoJSON Polygon
        const geoJsonLayer = L.geoJSON(zone.geoJsonPolygon, {
          style: {
            color: color,
      ...
      // We skip the custom marker for GeoJSON grids to avoid clutter, unless selected
      if (isSelected || !zone.geoJsonPolygon) {
