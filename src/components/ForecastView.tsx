...
import { HOURLY_FORECAST_DATA, HOURLY_FORECAST_48H } from '../data/mockData';
...
export const ForecastView: React.FC<ForecastViewProps> = ({ onOpenFullBrief }) => {
  const [timeHorizon, setTimeHorizon] = useState<'24h' | '48h'>('24h');
  const [selectedHour, setSelectedHour] = useState<HourlyForecast>(
    HOURLY_FORECAST_DATA.find((h) => h.isNow) || HOURLY_FORECAST_DATA[4]
  );

  const forecastData = timeHorizon === '24h' ? HOURLY_FORECAST_DATA : HOURLY_FORECAST_48H;
    ...
    <div id="forecast-view" className="max-w-7xl mx-auto space-y-8 pb-16 animate-in fade-in duration-300">
      ...
      <header className="space-y-1">
        ...
        <p className="text-sm text-[#4E6858]">
          24-Hour Predictive Risk Model · Northern Chennai Sector
        </p>
      </header>
        ...
        >
          ...
          <div className="bg-[#F8FAF8] border border-[#E2EBE5] rounded-xl p-4 space-y-3">
            ...
            <div className="flex items-center gap-1.5 overflow-x-auto pb-1">
              {forecastData.slice(0, 8).map((hour) => (
                <button
                  key={hour.time}
